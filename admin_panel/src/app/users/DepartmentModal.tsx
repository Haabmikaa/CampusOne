...
export default function DepartmentModal({ onClose }: { onClose: () => void }) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setIsAdding(true);
    try {
      await addDoc(collection(db, "departments"), { name: newName.trim() });
      setNewName("");
    } catch (e) {
      console.error(e);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this department? Users in this department will NOT be deleted.")) return;
    try {
      ...
      <div className="bg-[#161616] border border-[#2A2A2A] rounded-[2.5rem] w-full max-w-md overflow-hidden shadow-2xl relative">
        <div className="p-8 border-b border-[#2A2A2A] flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-bold text-stone-500 uppercase tracking-widest mt-1">For lecturers — e.g. Civil Engineering, Software Eng.</p>
          </div>
        ...
        <div className="p-8 space-y-6">
          <form onSubmit={handleAdd} className="flex gap-2">
            <input
              ...
              placeholder="New department (e.g. IT)"
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
            {loading ? (
              ...
              <p className="text-center py-10 text-stone-500 text-xs font-bold uppercase tracking-widest">No departments added yet</p>
            ) : (
              departments.map((dept) => (
                <div key={dept.id} className="flex items-center justify-between p-4 bg-black/40 border border-[#2A2A2A] rounded-2xl group hover:border-[#D4AF37]/30 transition-all">
                  <span className="text-sm font-bold text-white group-hover:text-[#D4AF37] transition-colors">{dept.name}</span>
                  <button
